...
> **Note (2026-06-09):** this is the original handoff bundle doc, kept for provenance. The repo has
> since been reorganised — files referenced below by flat name now live under `data/`, `sources/`,
> and `scripts/`, and the skill lives in-repo at `.claude/skills/` (not `/mnt/skills/user/`). See the
> top-level [`README.md`](../README.md) for the current layout and the [`docs/MASTER-PROMPT.md`](MASTER-PROMPT.md)
> spec. The provenance, status, and column definitions below are still accurate.
